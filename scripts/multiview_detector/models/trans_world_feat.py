...
from scripts.multiview_detector.models.transformer import TransformerEncoderLayer, TransformerEncoder
from scripts.multiview_detector.models.deformable_transformer import DeformableTransformerEncoderLayer, \
    DeformableTransformerEncoder
from scripts.multiview_detector.models.ops.modules import MSDeformAttn
from scripts.multiview_detector.utils.image_utils import array2heatmap
...
class DeformTransWorldFeat(nn.Module):
    def __init__(self, num_cam, Rworld_shape, base_dim, hidden_dim=128, dropout=0.1, nhead=8, dim_feedforward=512,
        ...
        self.lvl_embedding = nn.Parameter(torch.Tensor(1, num_cam, hidden_dim))
    ...
    def forward(self, x, visualize=False):
        B, N, C, H, W = x.shape
        x = self.downsample(x.reshape(B * N, C, H, W))
        _, _, H, W = x.shape

        src_flatten = x.view(B, N, C, H, W).permute(0, 1, 3, 4, 2).contiguous().view([B, N * H * W, C])
        lvl_pos_embed_flatten = (self.pos_embedding.to(x.device).flatten(2).transpose(1, 2).unsqueeze(1) + self.lvl_embedding.repeat(B, 1, 1).view([B, N, 1, C])).view([B, N * H * W, C])
        ...
        #         import os
        #         if not os.path.exists('/home/ubuntu/kaiyi/MV2/MVDeTr-main/logs/imgs'):
        #             os.makedirs('/home/ubuntu/kaiyi/MV2/MVDeTr-main/logs/imgs')
        #         visualize_img.save(f'/home/ubuntu/kaiyi/MV2/MVDeTr-main/logs/imgs/worldfeat{cam + 1}.png')
        #
