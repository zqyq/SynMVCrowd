...
class MVDeTr(nn.Module):
    def __init__(self, args, arch='resnet18', z=0, world_feat_arch='conv',
                 bottleneck_dim=512, outfeat_dim=64, droupout=0.5, num_cam=5, devices=('cuda:0', 'cuda:1')):
        super().__init__()
        self.args = args
        ...
        if arch == 'vgg19':
            self.base = vgg19(pretrained=False).features
        ...
        #     self.world_feat = DeformConvWorldFeat(self.num_cam, self.cropped_size, base_dim)
        # elif world_feat_arch == 'deform_trans':
        #     n_points = 4
        #     # reference_points = create_reference_map(dataset, n_points).repeat([dataset.num_cam, 1, 1, 1])
        #     self.world_feat2 = DeformTransWorldFeat(dataset.num_cam, self.cropped_size, base_dim,
        #                                            n_points=n_points, stride=2)
        ...
        # self.map_classifier_ot1 = nn.Sequential(nn.Conv2d(256 + 128, 256, 3, padding=1), nn.ReLU(),
        #                                         nn.Conv2d(256, 128, 3, padding=1), nn.ReLU(),
        #                                         nn.Conv2d(128, 1, 3, padding=1, bias=False)).to(self.devices[0])
        #
        # self.feat_down = nn.Sequential(nn.Conv2d(512, 256, 3, padding=1), nn.ReLU(),
        #                                         nn.Conv2d(256, 256, 3, padding=1), nn.ReLU(),).to(self.devices[0])
        #
        # self.mse_fea = nn.Sequential(nn.Conv2d(1, 128, 3, padding=1), nn.ReLU(),
        #                                         nn.Conv2d(128, 128, 3, padding=1), nn.ReLU(),).to(self.devices[0])
    ...
    def forward(self,
        ...
        # imgs = F.interpolate(imgs.squeeze(), (90, 160))
        ...
        #                                                   [imgs[cam:cam + 1].to(self.devices[0]),  # 'cuda:0'
        #                                                    camera_paras2[cam:cam + 1].to(self.devices[0]),  # 'cuda:0'
        #                                                    wld_map_paras.to(self.devices[0]),  # 'cuda:0'
        #                                                    hw_random.to(self.devices[0])  # 'cuda:0'
        #                                                    ])
        #     world_feature = (world_feature - world_feature.min()) / (world_feature.max() - world_feature.min())
        #     plt.imshow(world_feature.squeeze().permute(1, 2, 0).cpu().numpy())
        #     plt.show()
        ...
        for batch in range(B):
            ...
            if train:
                ...
                paras = [1, self.view_size, 1, [int(wld_map_paras[0, 3].item()), int(wld_map_paras[0, 4].item())]]
            ...
            for i in range(self.depth_scales):
                ...
                for cam in range(self.num_cam):
                    ...
                    world_feature = spatial_transoformation_layer(paras,
                                                                  [in_feat[cam:cam + 1].to(self.devices[0]),  # 'cuda:0'
                                                                   camera_paras[batch][cam:cam + 1].to(self.devices[0]),
                                                                   # 'cuda:0'
                                                                   wld_map_paras[batch:batch + 1].to(self.devices[0]),
                                                                   # 'cuda:0'
                                                                   hw_random[batch:batch + 1].to(self.devices[0])
                                                                   # 'cuda:0'
                                                                   ])
        ...
        if train:
            return world_heatmap_mse.reshape(B, self.patch_num, world_heatmap_mse.shape[-3],
                                             world_heatmap_mse.shape[-2],
                                             world_heatmap_mse.shape[-1]), img_res
            # , world_heatmap_ot.reshape(B, self.patch_num, world_heatmap_ot.shape[-3], world_heatmap_ot.shape[-2],
            #                     world_heatmap_ot.shape[-1]))
        else:
            world_heatmap_mse = torch.where(world_heatmap_mse < 0, 0, world_heatmap_mse)
            return world_heatmap_mse.reshape(1, 1, world_heatmap_mse.shape[-3], world_heatmap_mse.shape[-2],
                                             world_heatmap_mse.shape[-1]), img_res
            # , world_heatmap_ot.reshape(1, 1, world_heatmap_ot.shape[-3], world_heatmap_ot.shape[-2],
            #                     world_heatmap_ot.shape[-1]))

        # else:
        #     world_heatmap = torch.zeros((1, 1, H, W)).to(self.devices[0])
        #     for i in range(H // 200):
        #         for j in range(W // 200):
        #             p_world_heatmap = self.map_classifier_shot(wraped_feat[:, :, :, i * 200: (i + 1) * 200, j * 200: (j + 1) * 200])
        #             p_world_heatmap = torch.where(p_world_heatmap < 0, -p_world_heatmap, p_world_heatmap)
        #             world_heatmap[:, :, i * 200: (i + 1) * 200, j * 200: (j + 1) * 200] = p_world_heatmap
        #
        #     return world_heatmap.reshape(1, 1, world_heatmap.shape[-3], world_heatmap.shape[-2], world_heatmap.shape[-1]), img_res
