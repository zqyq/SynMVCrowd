...
def spatial_transoformation_layer_with_mask(paras, x):
    input_feature, input_camera_paras, input_wld_map_paras, input_hw_random = x
    mask_one_feat = torch.ones_like(input_feature)[:, :1, :, :]
    output = _transform(paras, x)
    mask_out = _transform(paras, [mask_one_feat, input_camera_paras, input_wld_map_paras, input_hw_random])

    output = output[0]
    output = output.clone().permute((0, 3, 1, 2))
    mask_out = mask_out[0]
    mask_out = mask_out.clone().permute((0, 3, 1, 2))
