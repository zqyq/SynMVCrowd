...
from scripts.multiview_detector.utils.projection import *
...
class frameDataset(VisionDataset):
    def __init__(self, base, train=True, transform=ToTensor(), target_transform=ToTensor(),
        ...
        # 相机位置参数
        self.cam_loc = self.base.calculate_camera_position  # [n_view,3]
        pass
    ...
    def __getitem__(self, index):
        ...
        for cam in range(self.num_cam):
            ...
            img_gt = np.stack([img_gt_head], axis=2)
            if self.reID:
                img_gt = (img_gt > 0).int()
            if self.target_transform is not None:
                img_gt = self.target_transform(img_gt)
            imgs_gt.append(img_gt.float())
        return imgs, imgs_gt, map_gt.float(), frame
...
def test():
    ...
    # from multiview_detector.mvdetr_datasets.MultiviewX import MultiviewX
    from scripts.multiview_detector.utils.projection import get_worldcoord_from_imagecoord
    dataset = frameDataset(Wildtrack(os.path.expanduser('/mnt/d/Datasets/Wildtrack')), train=True)
    # test projection
    world_grid_maps = []
