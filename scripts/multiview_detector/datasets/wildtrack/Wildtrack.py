...
class Wildtrack(VisionDataset):
    ...
    def get_worldgrid_from_worldcoord(self, world_coord):
        # mvdetr_datasets default unit: centimeter & origin: (-300,-900)
    ...
    def calculate_camera_position(self):
        ...
        for cam in range(self.num_cam):
            ext_mat = self.extrinsic_matrices[cam]
            R = ext_mat[:, :3]
            t = ext_mat[:, 3]
            C = -np.linalg.inv(R) @ t
            camera_positions.append(C)
        return torch.from_numpy(np.stack(camera_positions, axis=0)).float()  # [n_view,3]


def test():
    from scripts.multiview_detector.utils.projection import get_imagecoord_from_worldcoord
    dataset = Wildtrack(os.path.expanduser('~/Data/Wildtrack'), )
    pom = dataset.read_pom()
    print(pom[0][0])
    foot_3ds = dataset.get_worldcoord_from_pos(np.arange(np.prod(dataset.worldgrid_shape)))
    errors = []
    for cam in range(dataset.num_cam):
        ...
        for pos in range(np.prod(dataset.worldgrid_shape)):
            bbox = pom[pos][cam]
