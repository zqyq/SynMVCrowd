...
import numpy as np
import torch
import scripts.multiview_detector.datasets.citystreet.camera_proj_Zhang as proj
...
def proj_2Dto2D(input_shape, out_shape, devicenum, person_heights, proj_store_path):
    ...
    for view in range(1, 4):
        ...
        #         '/mnt/d/Yunfei/Study/Baseline_MVDet/Projection_files/Single_view_height_projeciton_relation',
...
def generate_cor_mask(out_shape, data_root, mask_store_path):
    ...
    for view in range(1, 4):
        ...
        if view == 1:
            view = 'view1'
            view_gp_mask = np.load(os.path.join(data_root, 'mask/view1_GP_mask.npz'))
        if view == 2:
            view = 'view2'
            view_gp_mask = np.load(os.path.join(data_root, 'mask/view2_GP_mask.npz'))
        if view == 3:
            view = 'view3'
            view_gp_mask = np.load(os.path.join(data_root, 'mask/view3_GP_mask.npz'))
        # gp view mask:
        view_gp_mask = view_gp_mask['arr_0']
        view_gp_mask = cv2.resize(view_gp_mask, (out_shape[1], out_shape[0]))
        view_gp_mask = torch.from_numpy(view_gp_mask).float()
