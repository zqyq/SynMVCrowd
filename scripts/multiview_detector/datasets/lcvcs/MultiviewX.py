...
class MultiviewX(VisionDataset):
    ...
    def get_intrinsic_extrinsic_matrix(self, camera_i):
        ...
        translation_matrix = np.array(tvec).reshape(3, 1)
...
def test():
    ...
    dataset = MultiviewX(os.path.expanduser('~/MultiviewX'), )
    pom = dataset.read_pom()

    for cam in range(dataset.num_cam):
        head_errors, foot_errors = [], []
        t = np.prod(dataset.worldgrid_shape)
        for pos in range(0, np.prod(dataset.worldgrid_shape), 16):
            bbox = pom[pos][cam]
