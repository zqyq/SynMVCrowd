...
class frameDataset(VisionDataset):
    def __init__(self, base, train=True, transform=ToTensor(), target_transform=ToTensor(),
        ...
        pass
        # 当前所需角度区域
        # self.degree_range = kwargs['degree_range']  # (0,60)
    ...
    def __getitem__(self, index):
        ...
        for p in label_path_list_sampling:
            ...
            with open(label_path_name, 'r') as data_file:
                coords_info = json.load(data_file)
            coords, coords_2d, paras, cam_loc = self.read_json_view(coords_info)
            # cam_deg = self.calculate_yaw(paras[7:10])
            # if cam_deg > self.degree_range[1]:
            #     print(f'Current view angle {cam_deg} exceeds the specified range {self.degree_range}. Resampling view.')
            #     continue
            cam_locations.append(cam_loc)
        ...
        for i in range(self.patch_num):
            ...
            # normalize
            heatmap = heatmap/ heatmap.sum() * GP_point_map[i].sum()
            GP_density_map.append(heatmap)

        GP_density_map = np.asarray(GP_density_map) * self.scale

        scene_frame_cam = f"{scene_i}_{frame_j}_{'_'.join(label_path_list_sampling)}"
        if self.train:
            return img_views, single_view_dmaps, camera_paras, wld_map_paras, hw_random, GP_point_map, GP_density_map, cam_location, scene_frame_cam
        else:
            heatmap_d = np.zeros_like(whole_pl_map)
            centers_d = np.transpose(np.nonzero(whole_pl_map))
            for center in centers_d:
                heatmap_d = draw_umich_gaussian(heatmap_d, center, 3)
            whole_pl_dmap = heatmap_d * self.scale
            return img_views, single_view_dmaps, camera_paras, wld_map_paras, hw_random, GP_point_map, whole_pl_dmap, cam_location, whole_pl_map
...
def test():
    ...
    dataset = frameDataset(
        Wildtrack(os.path.expanduser('/mnt/d/Datasets/Wildtrack')), degree_range=(0, 60))
